...
class SchedulingService:
    ...
    def suggest_alternative_times(
        ...
        for day_offset in range(days_to_check):
            ...
            for hour, minute in time_slots:
                ...
                # Check for conflicts
                conflicts = self.check_event_conflicts(household_id, slot_start, slot_end)
                ...
                if not conflicts["has_conflict"]:
                    suggestions.append({
                        ...
                        "time_description": slot_start.strftime("%A, %B %d at %I:%M %p")
                    })
    ...
    def get_optimal_task_assignment(
        ...
        """Find optimal user assignment considering conflicts and workload"""
        ...
        return {
            ...
            "assignment_reason": self._get_assignment_reason(user_scores[0] if user_scores else None)
        }
    
    def _calculate_assignment_score(self, user_id: int, due_date: datetime) -> Dict[str, Any]:
    ...
    def _get_assignment_reason(self, recommended_user: Dict[str, Any]) -> str:
        ...
        if recommended_user["conflicts"] == 0 and recommended_user["current_workload"] <= 2:
            return f"{recommended_user['user_name']} has no conflicts and light workload"
    ...
    def get_household_schedule_overview(
        ...
        for day_data in schedule_by_day.values():
            day_data["conflicts"] = len([
                task for task in day_data["tasks"] 
                if len([t for t in day_data["tasks"] if t["assigned_to"] == task["assigned_to"]]) > 1
            ])
        ...
        return {
            ...
            "busiest_day": max(schedule_by_day.values(), key=lambda x: len(x["events"]) + len(x["tasks"]))["date"].isoformat()
        }
