...
class TaskService:
    ...
    def create_task_with_rotation(
        self, task_data: TaskCreate, household_id: int, created_by: int
    ) -> Task:
        ...
        if task_data.due_date and self._has_task_conflict(
            ...
            assigned_to = self._get_next_assignee(
    ...
    def _get_next_assignee(self, household_id: int, exclude_user_id: int = None) -> int:
        ...
        query = self.db.query(User).filter(
            User.household_id == household_id, User.is_active == True
        )
    ...
    def get_user_task_score(
        self, user_id: int, month: int = None, year: int = None
    ) -> Dict[str, Any]:
    ...
    def get_household_leaderboard(
        self, household_id: int, month: int = None, year: int = None
    ) -> List[Dict[str, Any]]:
    ...
    def _create_recurring_instances(self, task: Task):
        ...
        # FIXED: Create only next 2-3 instances, not 12
    ...
    def update_task_status(self, task_id: int, new_status: str, user_id: int) -> Task:
        ...
        valid_transitions = {
            ...
            "in_progress": ["completed", "pending"],  # Can go back to pending
        ...
        if new_status not in valid_transitions.get(current_status, []):
            raise ValueError(f"Cannot change from {current_status} to {new_status}")
