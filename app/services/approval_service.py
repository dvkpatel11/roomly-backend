...
class ApprovalService:
    ...
    def approve_guest(self, guest_id: int, approver_id: int) -> Dict[str, Any]:
        ...
        if all_approved:
            ...
            pending_count = self._get_pending_guest_approvals_count(guest_id, guest.household_id)
            return {
                "success": True,
    ...
    def deny_guest(self, guest_id: int, denier_id: int, reason: str = "") -> Dict[str, Any]:
    ...
    def approve_event(self, event_id: int, approver_id: int) -> Dict[str, Any]:
        ...
        if all_approved:
            ...
            pending_count = self._get_pending_event_approvals_count(event_id, event.household_id)
            return {
                "success": True,
                "message": f"Approval recorded. {pending_count} more approvals needed",
                "fully_approved": False,
    ...
    def deny_event(self, event_id: int, denier_id: int, reason: str = "") -> Dict[str, Any]:
    ...
    def get_pending_guest_approvals(self, household_id: int) -> List[Dict[str, Any]]:
        ...
        for guest in pending_guests:
            pending_count = self._get_pending_guest_approvals_count(guest.id, household_id)
            result.append({
                "guest_id": guest.id,
    ...
    def get_pending_event_approvals(self, household_id: int) -> List[Dict[str, Any]]:
        ...
        for event in pending_events:
            pending_count = self._get_pending_event_approvals_count(event.id, household_id)
            result.append({
                "event_id": event.id,
    ...
    def _record_guest_approval(self, guest_id: int, user_id: int, approved: bool, reason: str = "") -> bool:
        """Record user's approval/denial of guest"""
        # Would update GuestApproval table
        return True
    
    def _record_event_approval(self, event_id: int, user_id: int, approved: bool, reason: str = "") -> bool:
    ...
    def _check_all_guest_approvals(self, guest_id: int, household_id: int) -> bool:
        """Check if all household members have approved guest"""
        # For now, simplified logic - in reality would query approval table
        household_member_count = self.db.query(User).filter(
            User.household_id == household_id,
            User.is_active == True
        ).count()
        
        # Simplified: assume approved if more than 1 day old (mock approval)
    ...
    def _get_pending_guest_approvals_count(self, guest_id: int, household_id: int) -> int:
    ...
    def _get_pending_event_approvals_count(self, event_id: int, household_id: int) -> int:
