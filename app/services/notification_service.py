...
from ..utils.constants import NotificationType, Priority
import asyncio
...
class NotificationService:
    ...
    def _should_send_email(self, notification_type: str, preferences: Dict) -> bool:
        ...
        email_map = {
            NotificationType.BILL_DUE.value: preferences.get("bill_reminders_email", True),
            NotificationType.BILL_OVERDUE.value: preferences.get("bill_reminders_email", True),
            NotificationType.TASK_OVERDUE.value: preferences.get("task_reminders_email", True),
            NotificationType.EVENT_REMINDER.value: preferences.get("event_reminders_email", True),
            NotificationType.GUEST_REQUEST.value: preferences.get("guest_requests_email", True),
            NotificationType.ANNOUNCEMENT.value: preferences.get("announcements_email", True),
        }
    ...
    def _should_send_push(self, notification_type: str, preferences: Dict) -> bool:
        ...
        push_map = {
            NotificationType.BILL_DUE.value: preferences.get("bill_reminders_push", True),
            NotificationType.BILL_OVERDUE.value: preferences.get("bill_reminders_push", True),
            NotificationType.TASK_OVERDUE.value: preferences.get("task_reminders_push", True),
            NotificationType.EVENT_REMINDER.value: preferences.get("event_reminders_push", True),
            NotificationType.GUEST_REQUEST.value: preferences.get("guest_requests_push", True),
            NotificationType.EXPENSE_ADDED.value: preferences.get("expense_updates_push", True),
        }
    ...
    def check_and_send_bill_reminders(self):
        ...
        for bill in bills:
            ...
            for due_date in [current_month_due, next_month_due]:
                ...
                if self._is_time_for_reminder(now, three_days_before, tolerance_hours=1):
                    self._send_bill_reminder(bill, due_date, household_members, "3 days")
                ...
                if due_date < now:
                    ...
                    if days_overdue > 0:
                        ...
                        daily_reminder_time = now.replace(hour=10, minute=0, second=0, microsecond=0)
                        if self._is_time_for_reminder(now, daily_reminder_time, tolerance_hours=1):
                            self._send_bill_overdue_reminder(bill, due_date, household_members, days_overdue)
    
    def _send_bill_reminder(self, bill: Bill, due_date: datetime, members: List[User], timing: str):
    ...
    def _send_bill_overdue_reminder(self, bill: Bill, due_date: datetime, members: List[User], days_overdue: int):
    ...
    def _send_task_overdue_reminder(self, task: Task, now: datetime):
        ...
        if days_overdue > 0:
            ...
            overdue_text = f"{int(hours_overdue)} hour{'s' if int(hours_overdue) > 1 else ''}"
    ...
    def check_and_send_event_reminders(self):
        ...
        for event in upcoming_events:
            ...
            if self._is_time_for_reminder(now, twenty_four_hours_before, tolerance_hours=1):
    ...
    def _send_event_reminder(self, event: Event, members: List[User], timing: str):
        ...
        for member in members:
            ...
            if timing == "24 hours":
                ...
                message = f"'{event.title}' starts in 2 hours ({event_time}). See you there!"
    ...
    # UTILITY METHODS
    def _is_time_for_reminder(self, now: datetime, target_time: datetime, tolerance_hours: int = 1) -> bool:
    ...
    def _get_current_month_due_date(self, bill: Bill, now: datetime) -> Optional[datetime]:
    ...
    def get_notification_summary(self, user_id: int) -> Dict[str, Any]:
        ...
        high_priority_count = self.db.query(Notification).filter(
            and_(
                Notification.user_id == user_id,
                Notification.is_read == False,
