...
class EventService:
    ...
    def create_rsvp(
        ...
        existing_rsvp = self.db.query(RSVP).filter(
            and_(
    ...
    def get_user_upcoming_events(self, user_id: int, days_ahead: int = 30) -> List[Dict[str, Any]]:
        ...
        rsvps = self.db.query(RSVP).join(Event).filter(
            and_(
                RSVP.user_id == user_id,
                RSVP.status == "yes",
    ...
    def get_event_statistics(self, household_id: int, months_back: int = 6) -> Dict[str, Any]:
        ...
        avg_attendance = total_attendance / events_with_rsvps if events_with_rsvps > 0 else 0
        ...
        most_popular_type = max(event_types, key=event_types.get) if event_types else None
        ...
        return {
            ...
            "events_per_month": len(published_events) / months_back if months_back > 0 else 0
        }
    
    def get_pending_events_for_approval(self, household_id: int) -> List[Dict[str, Any]]:
