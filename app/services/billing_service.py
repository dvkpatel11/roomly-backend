...
class BillingService:
    ...
    def _generate_bill_instances(self, bill: Bill, months_ahead: int = 3):
        ...
        for due_date in due_dates:
            ...
            existing_payment = self.db.query(BillPayment).filter(
                and_(
    ...
    def get_upcoming_bills(
        ...
        active_bills = self.db.query(Bill).filter(
            and_(
                ...
                if due_date < now:  # Past due
                    ...
                    if payment_status["total_paid"] < bill.amount:
                        overdue.append({
                            ...
                            "amount_remaining": bill.amount - payment_status["total_paid"],
                            "due_date": due_date,
    ...
    def _get_bill_payment_status(self, bill_id: int, month: str) -> Dict[str, Any]:
        ...
        payments = self.db.query(BillPayment).filter(
            and_(
                BillPayment.bill_id == bill_id,
                BillPayment.for_month == month
            )
        ).all()
    ...
    def get_bill_payment_history(
        ...
        payments = self.db.query(BillPayment).filter(
            BillPayment.bill_id == bill_id
        ).order_by(BillPayment.payment_date.desc()).limit(months_back).all()
    ...
    def get_household_billing_summary(self, household_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_overdue_amount": sum(bill["amount_remaining"] for bill in overdue_bills)
        }
