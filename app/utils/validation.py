...
class ValidationHelpers:
    ...
    @staticmethod
    def validate_amount(amount: float, max_amount: float = AppConstants.MAX_EXPENSE_AMOUNT) -> bool:
    ...
    @staticmethod
    def validate_split_ratios(ratios: Dict[int, float], total_amount: float) -> Dict[str, Any]:
        ...
        if all(0 <= ratio <= 100 for ratio in ratios.values()):
            ...
            if total_ratio > total_amount:
                errors.append(f"Total split amount exceeds expense amount: ${total_ratio} > ${total_amount}")
    ...
    @staticmethod
    def validate_recurring_pattern(pattern: str, start_date: datetime) -> Dict[str, Any]:
    ...
    @staticmethod
    def validate_guest_stay_duration(check_in: datetime, check_out: Optional[datetime] = None) -> Dict[str, Any]:
...
class CustomValidators:
    ...
    def validate_task_points(cls, v):
        ...
        if not AppConstants.MIN_TASK_POINTS <= v <= AppConstants.MAX_TASK_POINTS:
            raise ValueError(f'Points must be between {AppConstants.MIN_TASK_POINTS} and {AppConstants.MAX_TASK_POINTS}')
        return v
