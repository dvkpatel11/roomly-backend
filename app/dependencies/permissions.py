...
async def require_specific_household_access(
    household_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> tuple[User, int]:
    """Ensure user has access to specific household"""
    try:
        household_service = HouseholdService(db)
        if not household_service.check_member_permissions(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to verify household access",
        )
